...
void trackList2Video(std::vector< MidiTrack >& trackList, std::vector< std::vector< std::vector < unsigned char > > >& imageValues, int n, int nFrames, double midiUnitsPerFrame, std::vector< int >& barLines, bool renderFullFrame = false) {
	...
	if(doVideoBarLines){
		for (size_t t = 0; t < barLines.size(); t++) {//set BarLines
			...
			if ((barX >= (int)(n*midiUnitsPerFrame*midi2pix) && renderFullFrame) || ((barX>= (int)((n-1)*midiUnitsPerFrame*midi2pix) + nPixX) && !renderFullFrame)) {
				barX = barX % nPixX;
				for (int y = staffLines.at(9); y < staffLines.at(5); y++) {
					if(y>-1 && y<vidnPixY- (doMiniScore?miniScoreYPix :0)) setColor(staffColor[0], staffColor[1], staffColor[2], barX, y, imageValues);
				}
				for (int y = staffLines.at(4); y < staffLines.at(0); y++) {
					if (y>-1 && y<vidnPixY - (doMiniScore ? miniScoreYPix : 0)) setColor(staffColor[0], staffColor[1], staffColor[2], barX, y, imageValues);
				}
	...
	for (int i = 0; i < 12; i++) {
		std::vector< double > HSLvalues = RGBToHSL(colorArray[0][i], colorArray[1][i], colorArray[2][i]);
			...
			if (xStartInFrame <= xEndInFrame) {
				for (int x = xStartInFrame; x <= xEndInFrame; x++) {
					if (x == xStartInFrame || x==xEndInFrame) {
						for (int y = yStart - (int)(volScale*yNoteWidth); y < yEnd + (int)(volScale*yNoteWidth); y++) setColor(outlineColorArray[0][colorIndx], outlineColorArray[1][colorIndx], outlineColorArray[2][colorIndx], x, y, imageValues);
					}
					setColor(outlineColorArray[0][colorIndx], outlineColorArray[1][colorIndx], outlineColorArray[2][colorIndx], x, yStart - (int)(volScale*yNoteWidth), imageValues);
					setColor(outlineColorArray[0][colorIndx], outlineColorArray[1][colorIndx], outlineColorArray[2][colorIndx], x, yEnd + (int)(volScale*yNoteWidth) - 1, imageValues);
				}
			}
			else {//wrap around case
				for (int x = 0; x <= xEndInFrame; x++) {
