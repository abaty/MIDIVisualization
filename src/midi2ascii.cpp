...
/ABATY: see here:http://www.archduke.org/midi/instrux.html
*/
...
void err(const char* l) { fprintf(stderr, "Error: %s\n", l); exit(1); }
...
int Midi2Ascii(std::string inputFile, std::string outputFile) {
	...
	FILE* outFile;
	outFile = fopen(outputFile.data(), "w");
	...
	fpi = fopen(inputFile.c_str(), "rb"); if (!fpi) { fprintf(stderr, "Couldn't open %s\n", inputFile.c_str()); exit(1); }
	...
	while (nat > 0) {
		...
		if (raw) { //l1 += sprintf(l1, "DT "); 
			l1 += sprintf(l1,   "%d", tri[tr].dt);//sprfrac(l1, tri[tr].dt, tpc, 1, frpad); 
			//l1 += sprintf(l1,   "   ("); 
		}
		...
		if (sth == 8 || sth == 9) {
			...
			if (sth == 8 || (sth == 9 && u == 0)) {
				assert(raw);
				//l1 += sprintf(l1, "off");
				if (sth == 8 && u != 0x40)l1 += sprintf(l1,   " v=%d", u);
				l1 += sprintf(l1,   "\n");
				goto bp1;
			}
			if (raw) {//l1 += sprintf(l1,  "on");
			}
			else {
			...
			//if (u != tri[tr].pv) { l1 += sprintf(l1,  " v=%d", u); tri[tr].pv = u; }
			l1 += sprintf(l1,   " v=%d", u);
			if (!raw && i < non && nob[i].voff != 0x40)l1 += sprintf(l1,   " v=%d", nob[i].voff);
			l1 += sprintf(l1,   "\n");
	...
	bp1:
		...
		if (tri[tr].dt >= 0) { tri[tr].tt += tri[tr].dt; tri[tr].p = p; }
		else { tri[tr].a = 0; nat--; }
	bp2:
		if (sep) {
	...
	if (sep)for (tr = 0; tr < tracks; tr++) {
		...
		for (outbuf = outbuf0; outbuf->nx; outbuf = outbuf->nx) {
			if ((outbuf->all || outbuf->tr == tr) && outbuf->bn != prb) { fprintf(outFile, "\n"); prb = outbuf->bn; }
			if (outbuf->all && outbuf->tr != tr)fprintf(outFile, "# ");
			if (outbuf->all || outbuf->tr == tr)fprintf(outFile, "%s", outbuf->l);
		}
		fprintf(outFile, "\n");
	}

	std::cout << "Here 5" << std::endl;
	fprintf(outFile, "# Successfully parsed\n");
	fprintf(outFile, "# Duration = %g seconds\n", ert / 1000000);
	fclose(outFile);
	return 0;
}